...
/*
 * Array of keys which can be used as accelerators.
 * FIXME: In Glade-2 this used to be automatically generated with a 
 * perl script, which is probably a Good Idea (tm).
 * If someone with perl knowledge steps up to do this it would be cool.
 * For now I just copyed the array.
 */
...
glade_keys_dialog_get_type (void)
{
	...
	if (!type) {
		static const GTypeInfo info = {
			...
			(GClassInitFunc) glade_keys_dialog_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,
			sizeof (GladeKeysDialog),
			0,
			(GInstanceInitFunc) glade_keys_dialog_init
		};

		type = g_type_register_static (GTK_TYPE_DIALOG, "GladeKeysDialog", &info, 0);
	}
...
static void
gkd_populate_key_list (GtkListStore *store)
{
	GtkTreeIter iter;
	guint i;
	...
	i = 0;
	while (GladeKeys[i].name) {
		gtk_list_store_append (store, &iter);
		gtk_list_store_set (store, &iter, KEYS_COLUMN, GladeKeys[i].name, -1);
		i++;
	}
}

static void
gkd_on_row_activated (GtkWidget *widget,
		      gpointer *notused1, 
		      gpointer *notused2,
		      gpointer *dialog)
{
	g_return_if_fail (GLADE_IS_KEYS_DIALOG (dialog));
...
glade_keys_dialog_init (GladeKeysDialog *dialog)
{
	...
	/* model */
	dialog->store = gtk_list_store_new (N_COLUMNS, GTK_TYPE_STRING);
	gkd_populate_key_list (dialog->store);

	/* view */
	dialog->list = gtk_tree_view_new_with_model (GTK_TREE_MODEL (dialog->store));
	
	/* the view now holds a reference, we can get rid of our own */
	g_object_unref (G_OBJECT (dialog->store));

	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes ("Keys", renderer,
							   "text", KEYS_COLUMN,
							   NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (dialog->list), column);
	
	/* when a symbol is double-clicked we emit a OK response */
	g_signal_connect (G_OBJECT (dialog->list), "row-activated",
			  G_CALLBACK (gkd_on_row_activated), dialog);
	...
	gtk_container_add (GTK_CONTAINER (scrolled_win), dialog->list);
	gtk_box_pack_start (GTK_BOX (GTK_DIALOG (dialog)->vbox), scrolled_win, TRUE, TRUE, 0);

	/* set up dialog title, buttons, etc */
	gtk_window_set_title (GTK_WINDOW (dialog), "Select Accelerator Key");
...
glade_keys_dialog_new (void)
{
	...
	dialog = g_object_new (GLADE_TYPE_KEYS_DIALOG, NULL);
...
glade_keys_dialog_get_selected_key_symbol (GladeKeysDialog *dialog)
{
	...
	g_return_val_if_fail (GLADE_IS_KEYS_DIALOG (dialog), NULL);
	
	selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (dialog->list));

	if (gtk_tree_selection_get_selected (selection, NULL, &iter)) {
		gtk_tree_model_get (GTK_TREE_MODEL (dialog->store), 
				    &iter, KEYS_COLUMN, &key_sym, -1);
	}
