  ...
  if (unknown_objects)
    {
      ...
      for (l = classes; l; l = g_list_next (l))
        {
          if (g_list_previous (l))
            g_string_append_printf (missing_types, ", %s", (const gchar *)l->data);
          else
      ...
      g_string_printf (text,
                       ngettext ("Especially because there is %d object that can not be build with type: %s",
                                 "Especially because there are %d objects that can not be build with types: %s", unknown_objects),
                       unknown_objects,
                       missing_types->str);
